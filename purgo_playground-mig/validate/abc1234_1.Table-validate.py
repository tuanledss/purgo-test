...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_playground' AND table_name = 'abc1234_1'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_playground.[abc1234_1]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "abc1234_1".replace(" ", "_").lower()
