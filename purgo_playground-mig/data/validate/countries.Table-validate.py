...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_playground' AND table_name = 'countries'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_playground.[countries]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "countries".replace(" ", "_").lower()
