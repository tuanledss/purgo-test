...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_playground' AND table_name = 'd_product'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_playground.[d_product]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "d_product".replace(" ", "_").lower()
