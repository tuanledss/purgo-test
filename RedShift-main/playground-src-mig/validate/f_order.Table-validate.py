...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_playground' AND table_name = 'f_order'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_playground.[f_order]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "f_order".replace(" ", "_").lower()
