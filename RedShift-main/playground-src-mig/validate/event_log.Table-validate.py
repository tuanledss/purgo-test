...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_playground' AND table_name = 'event_log'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_playground.[event_log]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "event_log".replace(" ", "_").lower()
