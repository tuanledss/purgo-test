...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'tickit' AND table_name = 'sales'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "tickit.[sales]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "sales".replace(" ", "_").lower()
