...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'purgo_playground' AND table_name = 'f_events'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "purgo_playground.[f_events]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "f_events".replace(" ", "_").lower()
