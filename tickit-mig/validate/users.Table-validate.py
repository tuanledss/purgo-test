...
with redshiftEngine.connect() as connection:
	result = connection.execute("SELECT * FROM information_schema.columns WHERE table_schema = 'tickit' AND table_name = 'users'")
	for row in result:
...
redshiftCount = 0
redshiftTablename = "tickit.[users]"
with redshiftEngine.connect() as connection:
...
databricksCols = list()
databricksTablename = databricksCatalog + "." + databricksSchema + "." + "users".replace(" ", "_").lower()
